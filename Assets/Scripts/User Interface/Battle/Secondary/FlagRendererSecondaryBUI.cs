...
public class FlagRendererSecondaryBUI : PlayerSecondaryBUI
{
    ...
    {
        base.Update();
        targetHeightModifier = (state == UIState.DISABLING && Vector3.Distance(Camera.main.transform.position, managedPlayer.flagCameraPoint.transform.position) < 5.0f) ? (managedPlayer.boardCameraPoint.transform.position.y + 10) : (state == UIState.ENABLING) ? MiscellaneousVariables.it.boardUIRenderHeight : targetHeightModifier;
